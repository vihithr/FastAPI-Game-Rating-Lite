...
def get_ui_text() -> Dict[str, Any]:
    """获取UI文本配置"""
    return get_config().get("ui_text", {})


def get_quality_labels() -> Dict[str, str]:
    """获取品质评分标签"""
    return get_config().get("ratings", {}).get("quality", {}).get("labels", {})


def get_difficulty_labels() -> Dict[str, str]:
    """获取难度评分标签"""
    return get_config().get("ratings", {}).get("difficulty", {}).get("labels", {})


def get_quality_icon() -> str:
    """获取品质评分图标"""
    return get_config().get("ratings", {}).get("quality", {}).get("icon", "gem")


def get_difficulty_icon() -> str:
    """获取难度评分图标"""
    return get_config().get("ratings", {}).get("difficulty", {}).get("icon", "swords")


def get_quality_min() -> int:
    """获取品质评分最小值"""
    return get_config().get("ratings", {}).get("quality", {}).get("min", 1)


def get_quality_max() -> int:
    """获取品质评分最大值"""
    return get_config().get("ratings", {}).get("quality", {}).get("max", 10)


def get_difficulty_min() -> int:
    """获取难度评分最小值"""
    return get_config().get("ratings", {}).get("difficulty", {}).get("min", 1)


def get_difficulty_max() -> int:
    """获取难度评分最大值"""
    return get_config().get("ratings", {}).get("difficulty", {}).get("max", 60)
